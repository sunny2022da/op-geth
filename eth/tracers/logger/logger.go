...
func NewStepLogger() *stepLogger {
	l := &stepLogger{steps: 0}
	return l
}

func (t *stepLogger) CaptureStart(env *vm.EVM, from common.Address, to common.Address, create bool, input []byte, gas uint64, value *big.Int) {
}

// CaptureState also tracks SLOAD/SSTORE ops to track storage change.
func (t *stepLogger) CaptureState(pc uint64, op vm.OpCode, gas, cost uint64, scope *vm.ScopeContext, rData []byte, depth int, err error) {
	t.steps++
}

func (t *stepLogger) CaptureFault(pc uint64, op vm.OpCode, gas, cost uint64, scope *vm.ScopeContext, depth int, err error) {

}

func (t *stepLogger) CaptureEnd(output []byte, gasUsed uint64, err error) {
}

func (t *stepLogger) CaptureEnter(typ vm.OpCode, from common.Address, to common.Address, input []byte, gas uint64, value *big.Int) {
}

func (t *stepLogger) CaptureExit(output []byte, gasUsed uint64, err error) {}

func (*stepLogger) CaptureTxStart(gasLimit uint64) {}

func (*stepLogger) CaptureTxEnd(restGas uint64) {}

// Reset clears the data held by the logger.
func (t *stepLogger) Reset() {
	t.steps = 0
}

func (t *stepLogger) Steps() int {
	return t.steps
}
