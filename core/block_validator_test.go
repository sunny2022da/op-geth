...
func testHeaderVerification(t *testing.T, scheme string) {
	...
	chain, _ := NewBlockChain(rawdb.NewMemoryDatabase(), DefaultCacheConfigWithScheme(scheme), gspec, nil, ethash.NewFaker(), vm.Config{EnableParallelExec: true, ParallelTxNum: 8}, nil, nil)
	defer chain.Stop()
...
func testHeaderVerificationForMerging(t *testing.T, isClique bool) {
	...
	// Run the header checker for blocks one-by-one, checking for both valid and invalid nonces
	chain, _ := NewBlockChain(rawdb.NewMemoryDatabase(), nil, gspec, nil, engine, vm.Config{EnableParallelExec: true, ParallelTxNum: 8}, nil, nil)
	defer chain.Stop()
