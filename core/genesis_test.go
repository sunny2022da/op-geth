	...
	tests := []struct {
		...
		},
		{
			...
			fn: func(db ethdb.Database) (*params.ChainConfig, common.Hash, error) {
				...
				bc, _ := NewBlockChain(db, DefaultCacheConfigWithScheme(scheme), &oldcustomg, nil, ethash.NewFullFaker(), vm.Config{EnableParallelExec: true, ParallelTxNum: 8}, nil, nil)
				defer bc.Stop()
