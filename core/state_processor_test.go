...
func TestStateProcessorErrors(t *testing.T) {
	...
	{ // Tests against a 'recent' chain definition
		var (
			...
			blockchain, _  = NewBlockChain(db, nil, gspec, nil, beacon.New(ethash.NewFaker()), vm.Config{EnableParallelExec: true, ParallelTxNum: 8}, nil, nil)
	...
	// ErrTxTypeNotSupported, For this, we need an older chain
	{
		var (
			...
			blockchain, _ = NewBlockChain(db, nil, gspec, nil, ethash.NewFaker(), vm.Config{EnableParallelExec: true, ParallelTxNum: 8}, nil, nil)
		)
		defer blockchain.Stop()
		for i, tt := range []struct {
			txs  []*types.Transaction
