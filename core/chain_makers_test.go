...
func TestGeneratePOSChain(t *testing.T) {
	...
	blockchain, _ := NewBlockChain(db, nil, gspec, nil, beacon.NewFaker(), vm.Config{EnableParallelExec: true, ParallelTxNum: 8}, nil, nil)
...
func ExampleGenerateChain() {
	...
	// Import the chain. This runs all block validation rules.
	blockchain, _ := NewBlockChain(db, DefaultCacheConfigWithScheme(rawdb.HashScheme), gspec, nil, ethash.NewFaker(), vm.Config{EnableParallelExec: true, ParallelTxNum: 8}, nil, nil)
	defer blockchain.Stop()
