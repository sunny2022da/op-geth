...
func benchInsertChain(b *testing.B, disk bool, gen func(int, *BlockGen)) {
	...
	chainman, _ := NewBlockChain(db, nil, gspec, nil, ethash.NewFaker(), vm.Config{EnableParallelExec: true, ParallelTxNum: 8}, nil, nil)
	defer chainman.Stop()
	b.ReportAllocs()
	b.ResetTimer()
...
func benchReadChain(b *testing.B, full bool, count uint64) {
	...
	for i := 0; i < b.N; i++ {
		...
		chain, err := NewBlockChain(db, &cacheConfig, nil, nil, ethash.NewFaker(), vm.Config{EnableParallelExec: true, ParallelTxNum: 8}, nil, nil)
		if err != nil {
